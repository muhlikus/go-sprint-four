...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	// Проверять деление на 0 не будем потому что делим не на входной параметр
	avgSpeedInKmH := meanSpeed(action, duration)
	return runningCaloriesMeanSpeedMultiplier * avgSpeedInKmH * runningCaloriesMeanSpeedShift * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	avgSpeedInMS := meanSpeed(action, duration) * kmhInMsec
	heightInM := height / cmInM
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(avgSpeedInMS, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	avgSpeedInKmH := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (avgSpeedInKmH + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
